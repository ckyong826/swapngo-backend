...
func (b *withdrawBiz) ProcessWithdrawEvent(ctx context.Context, wUUID uuid.UUID, userID string, amountMYRC, amountMYR float64, bankName, bankAccountNo string) error {
	...
	// ==========================================
	// 步骤 A: 链上转账到国库，确认后才扣除账本余额
	// ==========================================
	// Transfer real MYRC to treasury on-chain first — the chain is the source
	// of truth. Only debit the ledger once the transfer confirms.
	txHash, err := b.tokenService.TransferToTreasury(ctx, withdrawal.AccountID.String(), amountMYRC)
	if err != nil {
		log.Printf("MYRC treasury transfer failed for withdraw %s: %v", wUUID, err)
	} else if debitErr := b.tokenService.DebitToken(ctx, withdrawal.AccountID.String(), models.MYRC, amountMYRC); debitErr != nil {
		err = debitErr
	}
